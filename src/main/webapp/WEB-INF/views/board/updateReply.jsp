						...
						<table>
							...
							<tr>
								...
								<td class="td" colspan="5">
									<c:if test="${!empty requestScope.board.board_original_img }"><!-- 첨부 파일이 있으면 다운로드 요청 -->
						...
						<c:forEach items="${requestScope.reply}" var="reply">
							<div id="replyList">
								<form action="${pageContext.request.contextPath}/updateReplyPro.do" method="post" onsubmit="return doAlert()">
									...
									<c:choose>
										<c:when test="${reply.reply_no == update}">
											<input type="submit" id="replyBtn" value="완료">
											<br><br>
											<input type="text" style="width:500px; height:100px;" name="updateReply" value="${reply.reply_content}">
										</c:when>
										<c:when test="${reply.reply_no != update}">
											<br><br>
											${reply.reply_content}
