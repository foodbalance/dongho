                ...
                <div class="row">
                    <div class="col-xl-9 mx-auto">
                        <div class="cta-inner bg-faded text-center rounded">
							...
							<c:if test="${ !empty e }">
								<h1 class="bubble site-heading-upper text-primary mb-3">${ message }</h1>
							</c:if>
							
							<!-- 비밀번호 찾기 완료, 가입, 탈퇴 안내-->
                            <c:set var="m" value="${ pwd }"/>
							<c:if test="${ !empty m }">
								<h1 class="bubble site-heading-upper text-primary mb-3">${ pwd }</h1>
							</c:if>
							<c:set var="m2" value="${ register }"/>
							<c:if test="${ !empty m2 }">
								<h1 class="bubble site-heading-upper text-primary mb-3">${ register }</h1>
							</c:if>
							<c:set var="m3" value="${ userdel }"/>
							<c:if test="${ !empty m3 }">
								<h1 class="bubble site-heading-upper text-primary mb-3">${ userdel }</h1>
							</c:if>
